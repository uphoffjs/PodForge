...
describe('usePauseTimer', () => {
  ...
  it('calls supabase.rpc("pause_timer") with correct args', async () => {
    mockRpc.mockResolvedValue({ error: null })

    const { result } = renderHook(() => usePauseTimer('evt1'), {
    ...
    expect(mockRpc).toHaveBeenCalledWith('pause_timer', {
      p_event_id: 'evt1',
      p_passphrase: 'secret',
    })
  })
  ...
  it('invalidates timer query on success', async () => {
    ...
    const { result } = renderHook(() => usePauseTimer('evt1'), { wrapper })
  ...
  it('shows generic error toast on other failures', async () => {
    ...
    expect(mockToastError).toHaveBeenCalledWith('Failed to pause timer')
  })
})
