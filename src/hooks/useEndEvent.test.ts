...
describe('useEndEvent', () => {
  ...
  it('calls supabase.rpc("end_event") with correct args', async () => {
    mockRpc.mockResolvedValue({ error: null })

    const { result } = renderHook(() => useEndEvent('evt1'), {
    ...
    expect(mockRpc).toHaveBeenCalledWith('end_event', {
      p_event_id: 'evt1',
      p_passphrase: 'secret',
    })
  })

  it('invalidates event query on success', async () => {
    mockRpc.mockResolvedValue({ error: null })
    ...
    const { result } = renderHook(() => useEndEvent('evt1'), { wrapper })
  ...
  it('shows success toast on success', async () => {
    ...
    expect(mockToastSuccess).toHaveBeenCalledWith('Event ended')
  })
