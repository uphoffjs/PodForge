...
describe('useResumeTimer', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('calls supabase.rpc("resume_timer") with correct args', async () => {
    mockRpc.mockResolvedValue({ error: null })

    const { result } = renderHook(() => useResumeTimer('evt1'), {
    ...
    expect(mockRpc).toHaveBeenCalledWith('resume_timer', {
      p_event_id: 'evt1',
      p_passphrase: 'secret',
    })
  })
  ...
  it('invalidates timer query on success', async () => {
    ...
    const { result } = renderHook(() => useResumeTimer('evt1'), { wrapper })
  ...
  it('shows generic error toast on other failures', async () => {
    ...
    expect(mockToastError).toHaveBeenCalledWith('Failed to resume timer')
  })
})
