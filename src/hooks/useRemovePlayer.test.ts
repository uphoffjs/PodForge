...
describe('useRemovePlayer', () => {
  ...
  it('calls supabase.rpc("remove_player") with correct args', async () => {
    mockRpc.mockResolvedValue({ error: null })

    const { result } = renderHook(() => useRemovePlayer('evt1'), {
    ...
    expect(mockRpc).toHaveBeenCalledWith('remove_player', {
      p_event_id: 'evt1',
      p_passphrase: 'secret',
      p_player_id: 'p1',
    })
  ...
  it('invalidates players query on success', async () => {
    ...
    const { result } = renderHook(() => useRemovePlayer('evt1'), { wrapper })
