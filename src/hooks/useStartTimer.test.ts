...
describe('useStartTimer', () => {
  ...
  it('calls supabase.rpc("start_timer") with correct args', async () => {
    mockRpc.mockResolvedValue({ error: null })

    const { result } = renderHook(() => useStartTimer('evt1'), {
    ...
    expect(mockRpc).toHaveBeenCalledWith('start_timer', {
      p_event_id: 'evt1',
      p_passphrase: 'secret',
    })
  })
  ...
  it('invalidates timer query on success', async () => {
    ...
    const { result } = renderHook(() => useStartTimer('evt1'), { wrapper })
  ...
  it('shows generic error toast on other failures', async () => {
    ...
    expect(mockToastError).toHaveBeenCalledWith('Failed to start timer')
  })
})
